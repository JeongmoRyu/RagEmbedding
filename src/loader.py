...
class Loader:
    ...
    def __init__(self, cfg):
        self.cfg = OmegaConf.load(cfg)["es"]
        self.es = Elasticsearch(
            hosts=[self.cfg.url],
            http_auth=(self.cfg.user, self.cfg.pw)
        )
    ...
    def inplace_docs(self, source, inplace=True):
        ...
        if inplace:
            ...
            self.es.delete_by_query(index=self.cfg.index_name,
            ...
            for chr in reserved_character:
                doc_name = doc_name.replace(f"{chr}", f"\\{chr}")
            res = self.es.count(index=self.cfg.index_name, body={'query':{'term':{'metadata.source.keyword':doc_name}}})["count"]
    ...
    def load_bulk(self, docs: List[Document]):
        ...
        Args:
            docs: docs
            inplace: if docs exist in index, replace docs with new one
        ...
        self.es.indices.refresh(index=self.cfg.index_name)
